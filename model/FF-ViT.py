...
from albumentations import (
    HorizontalFlip, HueSaturationValue, VerticalFlip, RandomResizedCrop, RandomBrightnessContrast, Compose, Normalize, CoarseDropout,
...
class CustomDatasetForFFVIT(Dataset):
    def __init__(self, csv_plant ,csv_disease, transforms, num_plant, num_disease, data_path):

        self.get_plant = 0
        self.get_disease = 0
        self.get_P_image = np.zeros((num_plant,1),dtype=int)
...
class CustomDatasetForTrainFFVIT(Dataset):
    def __init__(self, plant_csv_dir, disease_csv_dir, plant_image_dir, disease_image_dir, transform=None):
        ...
        self.plant_data = self._load_data(plant_csv_dir, label_idx=1)
        self.disease_data = self._load_data(disease_csv_dir, label_idx=1)
        self.epoch_pairs = []

        self.refresh_pairs()

    def _load_data(self, folder, label_idx):
    ...
    def __getitem__(self, idx):
        ...
        (plant_image_name, plant_label), (disease_image_name, disease_label) = self.epoch_pairs[idx]
        ...
        return transformed1, plant_label, transformed2, disease_label
...
class modelclassifierwithbase(nn.Module):
    def __init__(self,model_cls,model_dis,num_plant ,num_disease):
        ...
        self.linearc = nn.Linear(self.dim,num_plant,device=device)
        self.lineard = nn.Linear(self.dim,num_disease,device=device)
        
        # classifier for synthetic features
        self.linear_c = nn.Linear(768, num_plant)
...
def main():
    ...
    MODEL_PATH_plant = "path to your pretrained weight in pth"
    MODEL_PATH_disease = "path to your pretrained weight in pth"
    
    train_plant_list = './csv_ffvit/Train_plant'
    train_disease_list = './csv_ffvit/Train_disease'
    test_csv_path1 = './csv_ffvit/PV_seen_test.csv' 
    test_csv_path2 = './csv_ffvit/PV_unseen_test.csv'  
    ...
    train_transforms = Compose([
                RandomResizedCrop(
                                size=(img_size, img_size),  # tuple required
                                scale=(0.08, 1.0),          # default scale range
                                ratio=(3/4, 4/3),           # default aspect ratio
                                interpolation=1,             # optional
                                p=1.0
                                ),
                Transpose(p=0.5),
    ...
    train_dataset = CustomDatasetForTrainFFVIT(train_plant_list, train_disease_list, data_path, data_path, transform = train_transforms)
    ...
    test_loader1 = DataLoader(test_dataset1, batch_size=batch_size, num_workers=num_workers, shuffle=False)
    test_loader2 = DataLoader(test_dataset2, batch_size=batch_size, num_workers=num_workers, shuffle=False)
    ...
    if pretrained:
        ...
        model_cls.load_state_dict(torch.load(MODEL_PATH_plant),strict=False)
    ...
    if pretrained:
        ...
        model_dis.load_state_dict(torch.load(MODEL_PATH_disease),strict=False)
    
    model_cls.to(device)
    model_dis.to(device)
    
    model_classifier = modelclassifierwithbase(model_cls,model_dis,num_plant,num_disease)
